...
void GameState::saveGame(const std::string& filename) const {
    ...
    outFile << playerMap << botMap << playerShipManager << botShipManager << abilityManager << bot;
...
void GameState::loadGame(const std::string& filename) {
    ...
    inFile >> playerMap >> botMap >> playerShipManager >> botShipManager >> abilityManager >> bot;
...
std::ostream& operator<<(std::ostream& os, const GameState& gameState) {
    os << gameState.playerMap << gameState.botMap << gameState.playerShipManager
       << gameState.botShipManager << gameState.abilityManager << gameState.bot;
    return os;
}
